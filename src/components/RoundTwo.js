...
import RoundTwoCardLayout from './roundTwoComponents/RoundTwoCardLayout';
import RoundTwoImages from './roundTwoComponents/RoundTwoImages';
import GlobalUIAndStats from './GlobalUIAndStats';
import NextRoundButton from './NextRoundButton';
import {Link} from 'react-router-dom';
import './roundTwoComponents/RoundTwo.css';

//Parent Component for the Seconed Round.
function RoundTwo() {
  ...
  const resetRound = () => { //Resets the Round.
    const resetRound = [...RoundTwoImages, ...RoundTwoImages].sort(() => Math.random() - 0.5).map((card) => ({ ...card, id: Math.random() })) //Uses the same array twice, to make sure there is a pair of each card in the Cards game. Takes the cards and shuffles them around with the .sort while Math.random gives them a unique id/key each time it restarts.
  ...
  const turnsMax = 12; //Maximum number of turns before you lose.
  let turnsLeft = turnsMax - turns;
  
  if(turnsLeft === 0 && win === 8){ //If the amount of turns you have is less then one but you completed the win conditions, then you win.
  ...
  }else if(win === 8){ //The number of cards that need to be flipped around correctly before you win, basically the win conditions.
    showWinBox = 'showWinBox';
    score = 1*score + 1*turnsLeft;
    sessionStorage.setItem('sessionScore', score)
    hideGame = 'hideGameDiv';
    ...
    return (
      <div className='RoundTwoPage'>
        ...
        <div className='uiDiv'>
          ...
          <div className='RoundTwoTable'>
            <div className={showWinBox}>
              <h1>Congratulations You Win Round Two!</h1>
              <div className='nextRoundButton'>
                <NextRoundButton win={win} winAmount={8} goTo={'/RoundThree'} goText={'Go To Round 3'}/>
              </div>
            </div>
