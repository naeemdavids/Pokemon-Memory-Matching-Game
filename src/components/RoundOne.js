...
import RoundOneCardLayout from './roundOneComponents/RoundOneCardLayout';
import RoundOneImages from './roundOneComponents/RoundOneImages';
import GlobalUIAndStats from './GlobalUIAndStats';
import NextRoundButton from './NextRoundButton';
import {Link} from 'react-router-dom';
import './roundOneComponents/RoundOne.css';

//Parent Component for the First Round.
function RoundOne() {
    ...
    const resetRound = () => { //Resets the Round.
      const resetRound = [...RoundOneImages, ...RoundOneImages].sort(() => Math.random() - 0.5).map((card) => ({ ...card, id: Math.random() })) //Uses the same array twice, to make sure there is a pair of each card in the Cards game. Takes the cards and shuffles them around with the .sort while Math.random gives them a unique id/key each time it restarts.
    ...
    const turnsMax = 4; //Maximum number of turns before you lose.
    let turnsLeft = turnsMax - turns;


    if(turnsLeft === 0 && win === 4){ //If the amount of turns you have is less then one but you completed the win conditions, then you win.
    ...
    }else if(win === 4){ //The number of cards that need to be flipped around correctly before you win, basically the win conditions.
      showWinBox = 'showWinBox';
      score = 1*turnsLeft;
      sessionStorage.setItem('sessionScore', score)
      hideGame = 'hideGameDiv';
    ...
    return (
      <div className='RoundOnePage'>
        <div className='d-flex'>
          ...
          <div className='RoundOneTable'>
            <div className={showWinBox}>
              <h1>Congratulations You Win Round One!</h1>
              <div className='nextRoundButton'>
                <NextRoundButton win={win} winAmount={4} goTo={'/RoundTwo'} goText={'Go To Round 2'}/>
              </div>
            </div>
